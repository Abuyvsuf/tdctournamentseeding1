...
def run_by_region(entries, pools_per_language=None):
    """
    entries: [{"school", "region", "language", "category", "teams"}, ...]
    pools_per_language: dict like {"English": 4, "Kiswahili": 2}, the fixed
        business rule for how many pools each language gets per category.

    Returns: {
        region: {
            (language, category): {"coded": [...], "pools": {"Pool A": [...], ...}}
        }
    }
    """
    if pools_per_language is None:
        pools_per_language = {"English": 4, "Kiswahili": 2}

    regions = sorted(set(e["region"] for e in entries))
    result = {}
    for region in regions:
        region_entries = [e for e in entries if e["region"] == region]
        keys = sorted(set((e["language"], e["category"]) for e in region_entries))
        result[region] = {}
        for language, category in keys:
            num_pools = pools_per_language.get(language, 4)
            bucket_entries = [
                {"school": e["school"], "category": category, "teams": e["teams"]}
                for e in region_entries
                if e["language"] == language and e["category"] == category
            ]
            coded = code_teams(bucket_entries)
            pools = build_pools(coded, num_pools)
            result[region][(language, category)] = {
                "coded": [t["team_name"] for t in coded],
                "pools": pools,
            }
    return result
